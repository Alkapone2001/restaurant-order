...
async function createOrder(payload, user) {
  ...
  await query(
    ...
    [order.id, order.number, order.table, order.waiterId, order.status, order.paymentStatus, json(order.items, []), order.notes, order.discount, order.tax, order.service, json(order.payment, null), json(order.history, []), createdAt]
  );
...
async function updateOrderStatus(id, status, user) {
  ...
  await query("UPDATE orders SET status = $2, history = $3, updated_at = $4 WHERE id = $1", [id, status, json(order.history, []), order.updatedAt]);
