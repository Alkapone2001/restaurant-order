...
function addPizzaBrutForKitchen(items) {
  if (!kitchenNeedsPizzaBrut(items) || items.some(isAutoPizzaBrut)) return items;
  return items.concat(autoPizzaBrutItem());
}
...
async function appendItemsToOrder(order, items, payload, user, settings) {
  ...
  const nextItems = addPizzaBrutForKitchen(order.items.concat(items));
  const stationStatuses = { ...(order.stationStatuses || {}) };
  const touchedStations = Array.from(new Set(items.map(item => item.station).concat(!hadPizzaBrut && nextItems.some(isAutoPizzaBrut) ? ["pizza"] : [])));
...
async function createOrder(payload, user) {
  ...
  items = addPizzaBrutForKitchen(items);
