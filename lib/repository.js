...
function stationStatusesForItems(items, previousStatuses, updatedAt, user) {
  return Array.from(new Set((items || []).map(item => item.station))).reduce((statuses, station) => {
...
async function updateActiveOrder(id, payload, user) {
  ...
  if (!nonAutoItems.length) throw new Error("Active orders must keep at least one item.");
  ...
  const items = nonAutoItems.concat(autoItems);
