...
async function appendItemsToOrder(order, items, payload, user, settings) {
  ...
  const nextItems = order.items.concat(items);
  const stationStatuses = { ...(order.stationStatuses || {}) };
  const touchedStations = Array.from(new Set(items.map(item => item.station)));
  touchedStations.forEach(station => {
    stationStatuses[station] = { status: "sent", updatedAt, updatedBy: user.name };
  });
...
async function createOrder(payload, user) {
  ...
  const tableInput = String(payload.table || "").trim();
  const table = tableInput || "Pa tavoline";
  ...
  const openOrder = tableInput ? await findOpenTableOrder(table, user) : null;
...
async function markPaid(id, payload, user) {
  ...
  const order = await getOrder(id);
  if (!order) throw notFound("Porosia nuk u gjet.");
  if (user.role === "waiter" && order.waiterId !== user.id) throw forbidden("Kamarieret mund te mbyllin vetem porosite e tyre.");
