  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("end", () => {
      ...
      try {
        ...
        reject(new Error("JSON i pavlefshem"));
      }
    });
  });
}
...
async function requireUser(req, roles) {
  ...
  if (!user) {
    const error = new Error("Ju lutem kycuni.");
    error.status = 401;
    throw error;
  }
...
async function handleApi(req, res) {
  ...
  try {
    ...
    if (req.method === "GET" && pathname === "/public-settings") {
      const settings = await repo.getSettings();
      sendJson(res, 200, {
        restaurantName: settings.restaurantName || "Porosite e Restorantit"
      });
      return;
    }
    ...
    if (req.method === "PATCH" && stationStatusMatch) {
      const payload = await readBody(req);
      sendJson(res, 200, await repo.updateStationStatus(stationStatusMatch[1], stationStatusMatch[2], String(payload.status || ""), user));
      return;
    }
    ...
    sendJson(res, 404, { error: "Rruga e API nuk u gjet." });
  } catch (error) {
