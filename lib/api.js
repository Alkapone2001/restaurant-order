...
async function handleApi(req, res) {
  ...
  try {
    ...
    if (req.method === "GET" && pathname === "/bootstrap") {
      const [settings, users, products, orders, tableLocks] = await Promise.all([
        repo.getSettings(),
        (user.role === "admin" || repo.isHeadWaiter(user)) ? repo.listUsers() : [],
        repo.listProducts(),
        repo.listOrders(user),
        repo.listTableLocks()
      ]);
      sendJson(res, 200, {
        me: repo.cleanUser(user),
        settings,
        users: users.map(repo.cleanUser),
        products,
        orders,
        tableLocks
      });
    ...
    if (req.method === "GET" && pathname === "/push/public-key") {
      sendJson(res, 200, { publicKey: process.env.VAPID_PUBLIC_KEY || "" });
      return;
    }

    if (req.method === "POST" && pathname === "/push/subscribe") {
      sendJson(res, 201, await repo.savePushSubscription(user, await readBody(req)));
      return;
    }
    ...
    if (req.method === "GET" && pathname === "/table-locks") {
      sendJson(res, 200, await repo.listTableLocks());
      return;
    }
    ...
    if (req.method === "PATCH" && orderMatch) {
      sendJson(res, 200, await repo.updateActiveOrder(orderMatch[1], await readBody(req), user));
      return;
    }
    ...
    const stationStatusMatch = pathname.match(/^\/orders\/([^/]+)\/stations\/([^/]+)\/status$/);
    if (req.method === "PATCH" && stationStatusMatch) {
      const payload = await readBody(req);
      sendJson(res, 200, await repo.updateStationStatus(stationStatusMatch[1], stationStatusMatch[2], String(payload.status || ""), user, String(payload.batchId || "")));
      return;
    }
    ...
    if (productMatch && req.method === "DELETE") {
      sendJson(res, 200, await repo.deleteProduct(productMatch[1], user));
      return;
    }

    sendJson(res, 404, { error: "API route not found." });
  } catch (error) {
    const status = error.status || (error.code || error.name === "AggregateError" ? 503 : 400);
    if (status >= 500) console.error(`${req.method} ${pathname} failed:`, error);
    sendJson(res, status, { error: status >= 500 ? "Service temporarily unavailable." : error.message });
  }
}
